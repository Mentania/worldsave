...
public class MapReset {
	...
	public static void kickPlayers(World world) {
		for (Player pl : world.getPlayers()) {
			pl.teleport(Bukkit.getWorld("city").getSpawnLocation());
		}
	}

	public static Boolean worldSaved(String worldName) {
		File worldFolder = new File("plugins//ServerSystem//WorldSaves//" + worldName);
	...
	public static void saveWorld(World world) {
		if (world != null) {
			world.save();
			File worldFolder = new File("plugins//ServerSystem//WorldSaves//" + world.getName());
			File srcWorldFolder = new File(world.getName());
			if (worldFolder.exists()) {
				deleteFolder(worldFolder);
			}
			copyWorldFolder(srcWorldFolder, worldFolder);
			FileConfiguration settings = YamlConfiguration
					.loadConfiguration(new File(worldFolder, "WorldSettings.yml"));
	...
	public static void resetWorld(World world) {
		File srcWorldFolder = new File("plugins//ServerSystem//WorldSaves//" + world.getName());
		File worldFolder = new File(world.getName());
		if ((srcWorldFolder.exists()) && (worldFolder.exists()))
			if (world.getName().equals("city")) {
	...
	private static String getChunkGeneratorAsName(World world) {
		...
		for (Plugin plugin : Bukkit.getPluginManager().getPlugins()) {
			...
			if ((wc.generator() != null) && (world.getGenerator() != null)
					&& (wc.generator().getClass().getName().equals(world.getGenerator().getClass().getName()))) {
